        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <h2 className="text-3xl font-bold text-default mb-3">
              Війна впливає не лише на думки
            </h2>
            <p className="text-muted text-base max-w-lg mx-auto">
              Натисни на область, яка тебе турбує — щоб зрозуміти, що відбувається, і що з цим можна зробити
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {warEffects.map((item, i) => (
              ...
              >
                {/* Icon */}
                <span className={`inline-flex items-center justify-center w-12 h-12 rounded-xl text-2xl mb-4 ${item.iconBg} transition-transform duration-200 group-hover:scale-110`}>
                  {item.icon}
                </span>

                {/* Title */}
                <h3 className={`font-bold text-lg mb-1 ${item.accentColor}`}>
                  {item.title}
                </h3>
                ...
                {/* Arrow */}
                <div className={`mt-4 flex items-center gap-1 text-xs font-semibold ${item.accentColor} opacity-0 group-hover:opacity-100 transition-opacity duration-200`}>
                  <span>Читати</span>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="transform group-hover:translate-x-0.5 transition-transform">
