  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* Sticky header */}
          <div className={`bg-gradient-to-br ${item.headerGradient} px-8 pt-8 pb-6 shrink-0 border-b border-black/5 dark:border-white/10`}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-4">
                <span className={`text-3xl w-14 h-14 flex items-center justify-center rounded-2xl shrink-0 ${item.iconBg}`}>
                  {item.icon}
                </span>
                <div>
            ...
            {/* Intro */}
            <p className={`mt-5 text-sm leading-relaxed font-medium ${item.accentColor} bg-white/60 dark:bg-white/[0.06] rounded-xl px-4 py-3 border border-black/5 dark:border-white/10`}>
              {item.article.intro}
            </p>
          </div>
      ...
      {block.type === "list" && (
        <ul className="space-y-2">
          {block.items.map((item, i) => (
            <li key={i} className="flex gap-3 text-sm text-gray-700 dark:text-slate-200 leading-relaxed">
