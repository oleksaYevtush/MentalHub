    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* Header */}
          <div
            className={`bg-gradient-to-r ${symptom.color.split(' border')[0]} rounded-t-3xl px-8 pt-8 pb-6 border-b border-black/5 dark:border-white/10`}
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-4">
                <span
                  className={`text-4xl w-16 h-16 flex items-center justify-center rounded-2xl ${symptom.iconBg}`}
                >
                  {symptom.icon}
                </span>
                <div>
                  <h2 className={`text-2xl font-bold ${symptom.accentColor}`}>{symptom.title}</h2>
          ...
          <div className="px-8 py-6 space-y-6">
            ...
            <Block icon="🔍" title={symptom.meaning.title} badgeColor={symptom.badgeColor}>
              <ul className="space-y-3">
                {symptom.meaning.content.map((text, i) => (
                  <li
                    key={i}
                    className="flex gap-3 text-gray-700 dark:text-slate-200 leading-relaxed text-sm"
                  >
            ...
            <Block icon="✅" title={symptom.actions.title} badgeColor={symptom.badgeColor}>
              <ul className="space-y-3">
                {symptom.actions.items.map((item, i) => (
                  <li
                    key={i}
                    className="flex gap-3 text-gray-700 dark:text-slate-200 text-sm leading-relaxed"
                  >
...
function Block({ icon, title, badgeColor, accent = false, children }) {
  return (
    <div
      className={`rounded-2xl p-5 ${accent ? 'bg-red-50/50 border border-red-100 dark:bg-red-950/20 dark:border-red-900/30' : 'bg-gray-50/80 border border-gray-100 dark:bg-white/[0.04] dark:border-white/[0.08]'}`}
    >
